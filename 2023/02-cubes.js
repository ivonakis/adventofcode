const input = `Game 1: 14 green, 8 blue, 9 red; 5 blue, 4 green, 2 red; 4 red, 4 blue, 4 green; 1 blue, 3 green, 2 red; 10 red, 3 blue, 15 green; 2 red, 6 green, 3 blue
...
const gamesInput = input.split('\n');
var result1 = gamesInput.reduce((total, gameInput, index) => {
    const game = gameInput.split(':');
    const sets = game[1].split(';');
    const gameIsPosible = sets.every(set => {
        const cubesByColor = set.split(',');
        return cubesByColor.every(colorInput => {
            var cube = colorInput.trim().split(' ');
            return limits[cube[1]] >= parseInt(cube[0]);
        })
    })
    if (gameIsPosible) {
        total += index + 1;
...
var result2 = gamesInput.reduce((total, gameInput, index) => {
    const game = gameInput.split(':');
    const sets = game[1].split(';');
    const minCubes = sets.reduce((minCubes, set) => {
        const cubesByColor = set.split(',');
        return cubesByColor.reduce((minCubes, colorInput) => {
            var cube = colorInput.trim().split(' ');
            const numOfCubes = parseInt(cube[0]);
            if (!(cube[1] in minCubes) || minCubes[cube[1]] < numOfCubes) {
                minCubes[cube[1]] = numOfCubes;
            }
            return minCubes;
        }, minCubes)
    }, {})

    return total + Object.keys(minCubes).reduce((prev, key)=>{
        return prev * minCubes[key];
    }, 1);
}, 0);
