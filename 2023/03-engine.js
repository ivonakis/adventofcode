...
const regex = /\d+/g;
const symbol= /[^.\d]/;
const matches = inputLines.reduce((linesMatches, line, lineIndex) => {
    const lineMatches = Array.from(line.matchAll(regex)).reduce((matches, match)=> {
        var a  = match;
        matches.push({
            line: lineIndex,
            col: a.index,
    ...
    return lineMatches? linesMatches.concat(lineMatches) : linesMatches;
}, []);

const parts = matches.filter(match => {
    startIndex= match.col -1;
    endIndex = match.col + match.partNumber.length + 1;
    for (let i = -1; i < 2; i++) {
        const line = inputLines[match.line + i];
        const range = line.substring(startIndex, endIndex);
        const isPart = symbol.test(range);
        if (isPart) {
            return true;
        }
    }
...
const partSum = parts.reduce((total, part)=> total + parseInt(part.partNumber), 0);
console.log('Part 1: ' + partSum)

const gearScore = inputLines.reduce((total, line, lineIndex) => {
    const stars = Array.from(line.matchAll(/\*/g));
    if (stars) {
        return total + stars.reduce((total, star)=> {
            //Filter parts next to star
            let starParts = parts.filter(part => {
                return part.line >= lineIndex - 1 
                    && part.line <= lineIndex + 1
                    && star.index >= part.col - 1
                    && star.index <= part.col + part.partNumber.length
            });
            //Filter gears
            if (starParts.length > 1) {
                total += starParts.reduce((gearRatio, gear) => {
                    return gearRatio * parseInt(gear.partNumber);
                }, 1)
            }
            return total;
        }, 0);
